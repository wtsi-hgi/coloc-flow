...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/coloc/usage](https://nf-co.re/coloc/usage)
...
```console
nextflow run nf-core/coloc --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/coloc releases page](https://github.com/nf-core/coloc/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
