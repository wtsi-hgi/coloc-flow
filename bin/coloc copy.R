  ...
  if(nrow(t3)>0){
      ...
      for (qtl1 in all_uq){
        ...
        if (min(single_eqtl$p)<5e-4){
          fitted_eqtl = susie_rss_fit(single_eqtl)
          # print(summary(fitted_eqtl))
          # if (length(fitted_eqtl$sets$cs)>0){
              ...
              jpeg(paste('',variant_id,'_',qtl1,'_',chromosome1,'rplowt.jpg',sep=''))
                ...
                plot(single_eqtl$base_pair_location,-log10(single_eqtl$p),col=ifelse(single_eqtl$base_pair_location %in% c(base_pair_location$base_pair_location), 'red', 'black'),pch=20,xlim = c(range_min$base_pair_location-1000000,range_max$base_pair_location+1000000))
              dev.off()
